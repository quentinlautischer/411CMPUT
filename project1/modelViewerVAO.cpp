...
void setup(void)
{
  ...
  glBufferData(GL_ARRAY_BUFFER, sizeof(model->vertices_), NULL, GL_STATIC_DRAW);

  glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(model->vertices_), model->vertices_.data());

  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, buffer[INDICES]);
  glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(model->faces_), model->faces_.data(), GL_STATIC_DRAW);
...
int main(int argc, char **argv)
{
  ...
  glutCreateWindow("Project 1 - ModelViewer VAO - Quentin Lautischer");
